...
public class TicketMasterAPI {
  ...
  private static final String API_KEY = "JiSXI8oODx2GeQ2HhoChHjCV8TdzA71E";

  // We can change the return type to List for multiple address
  private String getAddress(JSONObject event) throws JSONException {
    if (!event.isNull("_embedded")) {
      ...
      if (!embedded.isNull("venues")) {
        ...
        for (int i = 0; i < venues.length(); i++) { // we only need the first one
  ...
  private List<Item> getItemList(JSONArray events) throws JSONException {
    ...
    for (int i = 0; i < events.length(); i++) {
      ...
      if (!event.isNull("rating")) {
        builder.setUrl(event.getString("rating"));
      }
  ...
  public List<Item> search(double lat, double lon, String keyword) {
    ...
    String query =
        String.format("apikey=%s&geoPoint=%s&keyword=%s&radius=%s", API_KEY, geoHash, keyword, 50);
    try {
      ...
      // handle all exceptions, do some operations
      e.printStackTrace();
    }
    return new ArrayList<>();
  }
